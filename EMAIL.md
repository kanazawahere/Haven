...
- ⏭ **Next:** CP-5 device verification — **Dovecot-in-proot** (chosen: CI-safe, no creds). Stand up Dovecot in an Arch/Debian guest, add an IMAP EMAIL profile, connect, drive `list_mail_folders`/`list_mail_messages`/`read_mail_message` over MCP, confirm `list_sessions` shows the IMAP `MAIL` session, and run the blackhole-tunnel negative test. Then CP-6 (SMTP send) + CP-7 (compose UI). Outstanding: optional in-process GreenMail CI test for the IMAP mapping; 11-locale translation of the new CP-4 strings.
