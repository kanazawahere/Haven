...
5. **Per-call consent.** Every tool carries one of the three consent levels this page
   is grouped by. Consent sheets are rendered by Haven itself, and agent-injected taps
   are refused while one is showing — an agent cannot approve its own prompt.
...
## Tool count by consent level

- **Asks every call**: 59 tools
- **Asks once per session**: 47 tools
- **No per-call prompt**: 76 tools

## Asks every call

Side-effectful or sensitive actions. Haven shows a consent sheet describing the specific action (paths, recipients, commands) and waits for your tap on every single call.
...
Reversible actions and screen-reading. Haven prompts the first time an agent session uses the tool; further calls in the same session proceed without re-prompting.
...
Read-only queries and tap-equivalent UI actions with no destructive effect. These still sit behind the endpoint being enabled and the client being paired.
