...
- **GL acceleration — shipped virgl path, the GL-2.1 ceiling, and the venus deep dive (verified on a Pixel 8 Pro).** The cage GL broker shipped (v5.59.63): app GL in a Sway cage runs on Mali (`GL_RENDERER = virgl (Mali-G715)`) instead of llvmpipe. But virgl caps *desktop* GL at **2.1** — a hard ceiling, because Android's EGL exposes only **GLES** and virgl's GLES→desktop-GL *compatibility* profile is fixed at 2.1, so glxgears, older games, and most CAD viewers can't exceed it. An **ES2→ES3 host-context bump** (`vrend_winsys_egl.c`, rebuilt + device-tested) was a **verified dead-end** — it didn't raise the desktop-GL version (a higher GLES context only lifts *GLES-native* caps, which the software-Xwayland cage couldn't even measure: `glxinfo` → `X_GetImage BadMatch`, `eglinfo` hangs). The forward path is **venus** (pass the device's real Mali Vulkan into the guest) **+ Mesa zink** (GL-on-Vulkan → modern GL). Deep-dived over 2026-06-15; current state:
  ...
  - **Rendering blocker — three stacked issues, all now root-caused, two *fixed* (deep dives, 2026-06-15).** What looked like one "fence gap" was three distinct layers; peeling them in order got a real GL app from *nothing* to *sustained on-Mali rendering*, and pinned the last cleanly to the cage compositor:
  ...
  - **Layer 3 — the remaining frontier, now root-caused: the cage compositor has no dma-buf import.** Apps still can't *present* — but it is **not** a hang and **not** a venus bug. The cage's Sway runs `WLR_RENDERER=pixman` (software) on the headless backend, so it advertises **no `zwp_linux_dmabuf_v1`** (the import global a GPU swapchain needs — only `wl_shm` and a screencopy *export* manager). A venus/zink swapchain therefore fails at **creation**: `vkCreateSwapchain`/surface queries return `VK_ERROR_SURFACE_LOST_KHR` (device-checked — `vkcube-wayland` on the venus GPU asserts at `demo_init_vk_swapchain`; `vulkaninfo` → `vkGetPhysicalDeviceSurfacePresentModesKHR` SURFACE_LOST) — *before* any swapchain image is allocated, so the AHB allocator is exonerated and venus advertises every needed dma-buf extension. The software-Vulkan `llvmpipe` device presents fine on the same compositor via `wl_shm`, proving the gap is GPU-present-specific. The compositor can't simply switch renderers: `WLR_RENDERER=gles2` fails immediately (`drmGetDevices2: Permission denied` → `no DRM FD available`) because wlroots' GPU renderers need a `/dev/dri` render node proot lacks, and the virgl broker is a userspace vtest socket, not a DRM node. **The unlock is therefore the GLES/dma-buf-import cage compositor below** (or brokering a DRM render node into proot, or a venus `wl_shm` CPU-copy present fallback) — not a venus/AHB change. **Shipped from this work:** `VN_PERF=no_fence_feedback` is now wired into both GPU cage launch paths (commit `12c72648`, device-verified). **Status:** the AHB allocator + the FORTIFY fix + fence/crash diagnostics remain uncommitted working-tree changes (recoverable from the `wayland-android` submodule + `/tmp/virgl-ahb` + `venusfence.c`); releases build the clean shipped virgl `.so` and are unaffected.
- **GLES-renderer cage compositor** — `WLR_RENDERER=gles2` wlroots over virgl, so the *compositor* (not just the app) is GPU-accelerated (full-pipeline present, not just the GL client). The virgl broker is GLES-only and does **not** unblock the GLES-only nested compositors (Hyprland/aquamarine, niri/smithay still fail `CBackend::create()` — they need a DRM render node, not a GL client driver).
